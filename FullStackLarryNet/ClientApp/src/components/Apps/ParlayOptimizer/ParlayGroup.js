    ...
    <div className='parlay-group'>
      <div className='parlay-header'>
        <div className='parlay-title-actions'>
          ...
          <button className="po-button" type='button' onClick={() => onAddLeg(parlay.id)}>
            Add Leg
          </button>
        </div>
      ...
      <div className='odds-group'>
        {parlay.legs.flatMap((leg, legIndex) => {
          const isOptionalLeg = legIndex >= 2;
          ...
          return [
            ...
            <input
              ...
              step='1'
              value={leg}
              onChange={(event) => onLegChange(parlay.id, legIndex, event.target.value)}
            />,
            isOptionalLeg ? (
              <button
              ...
              <span key={`${parlay.id}-spacer-${legIndex}`} className='leg-action-spacer' />
            ),
          ];
        })}
      </div>
